...
在找到交易环后，需要进行去交易环处理。假设$H(v)$中交易金额最小的交易为 $(s_m, t_m, w_m, \tau_m)$，即
\[
\forall (s, t, w, \tau) \in \mathcal{T} : w \le w_m
\]
\noindent 然后，需要$H(v)$中所有的交易减去该最小交易量，如果新的交易量为0，则移除该交易，即
\[
...
\]
\begin{align}
\Theta^{\prime}(t_0)=\Theta(t_0)-H(v) \cup \{\mathcal{E}(t), t\in\mathcal{T}\}
\end{align}
...
记节点$v$的转入金额为$p(v)$，则
\begin{align}
p(v) = \sum_{(s_i, v, w_i, \tau_i) \in \Theta^{\prime\prime}(t_0)}{w_i}
\end{align}
\noindent 同理，节点$v$的转出金额有
\begin{align}
q(v) = \sum_{(v, t_i, w_i, \tau_i) \in \Theta^{\prime\prime}(t_0)}{w_i}
\end{align}
\noindent 由此，
