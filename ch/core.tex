...
\begin{align}
\mathcal{C}(a) = \Omega(\beta(a)) * \Psi(\gamma(a))
\label{eq:rank}
\end{align}
...
\[
\mathcal{G}(v) = (p(v) + q(v)) * e^{-2\sin^2{(\frac{\pi}{4} - \arctan\frac{q(v)}{p(v)})}}
\]
\begin{align}
\gamma(v) = (\frac{\theta*\mathcal{G}(v)}{\mathcal{G}(v) + \mu})^{\lambda}
\end{align}
...
\begin{align}
\mathcal{C}(v) =  \frac{\beta(v)}{1+e^{a + b * \beta(v)}} * \frac{\gamma(v)}{1+e^{c + d * \gamma(v)}}
\end{align}
