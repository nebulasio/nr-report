...
{\color{red}相比白皮书~\cite{Nabulas}中对于Nebulas Rank的计算方法，我们做了如下改动：1.在构造交易图时取消了采取最高$K$个交易额作为权值；2.取消了LeaderRank中通过计算与背景节点有向边权重来获取重要性评分的方式。首先我们在计算出入度指标$\beta$时采用了去环处理，因此已经可以抵抗环形转账攻击同时保留边的强度信息。同时LeaderRank构造矩阵后需要经过若干次迭代直至收敛计算出重要性评分，当网络稀疏时该计算尚可接受，但随着交易的增加会使计算开销增加，这与我们在后文\refsec{sec:onchain?}的设定不符。在\refsec{sec:function}中我们提出的计算函数可以压制伪造低收入节点的收益。}
...
\noindent 如~\ref{fig:loop}所示，其中包含了一个交易环，注意，其中$(v_1, v_2, 100, 5)$并不包含在交易环中。
...
\end{align}
\noindent {\color{red} 证明：不难求得当$x>0$时，$f(x)$的一阶导数$f'(x)$恒大于$0$，且$\lim\limits_{x \to \infty} f'(x)=1$}
...
\reffig{fig-eth-simu}表示了以太坊的市值与星云指数随时间变化趋势，如图所示，星云指数能够有效反映以太坊市值变化，二者的相关系数（Correlation coefficient）为0.84427，$p$值（p-value）为$4.48\times{}10^{-17}$。说明函数~\ref{eq:rank}能够有效表示用户对整个经济体的贡献。
